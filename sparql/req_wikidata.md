Notre requête pour sortir ce dont on a besoin de wikidata.
**Attention :** il faut qu'on corrige les `filter`.
...
select ?name ?nameLabel ?seriesLabel ?platformLabel (year(?pub_date) as ?pub_year) ?genreLabel ?publisherLabel ?developerLabel ?image ?website ?esrbLabel ?pegiLabel ?bnf ?freebase 
{
  ...
  #filter(?pub_date > "1979-12-31T00:00:00Z"^^xsd:dateTime)
  #filter(?pub_date < "2017-01-01T00:00:00Z"^^xsd:dateTime)
